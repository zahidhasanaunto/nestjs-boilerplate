...
export class LoggerService {
  public static DEFAULT_TIME_FORMAT = 'DD-MM-YY HH:mm:ss';
  ...
  public static getLoggers(
    transportNames: LoggerTransport[],
  ...
  public static console(options?: LoggerOptions): ConfiguredTransport {
    ...
    const consoleTransportOptions = Object.assign(
      defaultOptions.consoleOptions,
      consoleLoggerOptions.consoleOptions
    );
  ...
  public static rotate(options?: LoggerOptions): ConfiguredTransport {
    ...
    const fileTransportOptions = Object.assign(
  ...
  debug(msg: any, context?: string) {
    this.logger.debug(this.dataToString(msg), [
      { context, reqId: this.requestId },
    ]);
  }

  info(msg: any, context?: string) {
    this.logger.info(this.dataToString(msg), [
      { context, reqId: this.requestId },
    ]);
  }

  warn(msg: any, context?: string) {
    this.logger.warn(this.dataToString(msg), [
      { context, reqId: this.requestId },
    ]);
  }
  ...
  private dataToString(msg: any) {
    ...
    if (
      typeof msg.entries === 'function' &&
      typeof msg.forEach === 'function'
    ) {
      const elements = [];
